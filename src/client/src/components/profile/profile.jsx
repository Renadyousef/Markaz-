...
export default function Profile() {
  ...
  function handleFirstNameChange(value) {
    ...
    setFirstName(value);
    if (value !== clean) {
      setFirstNameError("❌ يُسمح فقط بالحروف العربية، المسافة، أو الأرقام العربية (٠–٩).");
  ...
  function handleLastNameChange(value) {
    ...
    setLastName(value);
    if (value !== clean) {
      setLastNameError("❌ يُسمح فقط بالحروف العربية، المسافة، أو الأرقام العربية (٠–٩).");
    } else if (clean.trim().length < 2) {
  ...
  return (
    <div className="profile-onepage" dir="rtl">
        ...
        <div className="card-area stack">
          ...
          <form className="row g-3" onSubmit={handleSave}>
            <div className="col-12 col-md-6">
              <label className="form-label">الاسم الأول <span className="text-danger">*</span></label>
              <input
              ...
              {!firstNameError && (
                <div className="form-text">
                  يُسمح بالحروف العربية، المسافة، والأرقام العربية (٠–٩) فقط. الحد الأدنى حرفان.
                </div>
              )}
              {firstNameError && <div className="invalid-feedback d-block">{firstNameError}</div>}
            </div>

            <div className="col-12 col-md-6">
              <label className="form-label">اسم العائلة <span className="text-danger">*</span></label>
              <input
              ...
              {lastNameError && <div className="invalid-feedback d-block">{lastNameError}</div>}
            </div>
      ...
      <style>{`
        ...
        .avatar-wrap{ position:absolute; top: calc(28vh - 66px); left:50%; transform:translateX(-50%); width:132px; height:132px; }
        .avatar{ width:100%; height:100%; border-radius:50%; border:6px solid #fff; object-fit:cover; box-shadow:0 6px 18px rgba(0,0,0,.18); background:#eee; }
        .p-name{ font-weight:800; font-size:22px; margin:10px 0 2px; }
        .p-email{ opacity:.85; font-size:14px; margin-bottom:16px; }
        .stack{ width:clamp(280px, 90vw, 900px); }
        .card-area{
